      ...
      <div className="bg-white border border-slate-200 rounded-3xl p-8 shadow-sm space-y-6 mb-8">
        <div className="flex items-center justify-between mb-2">
          ...
          <button 
            ...
            className="px-3 py-1.5 bg-white border border-slate-200 text-slate-600 rounded-lg font-bold hover:bg-slate-50 hover:border-indigo-200 hover:text-indigo-600 transition-all shadow-sm flex items-center space-x-2 text-[10px] uppercase tracking-wider"
          >
                ...
                {BREAKPOINTS.map(b => (
                  <button 
                    ...
                    className={`flex-1 py-2 text-xs font-bold transition-all rounded-xl ${
                      s.contentMaxWidth === b.key ? 'bg-white text-indigo-600 shadow-sm' : 'text-slate-500 hover:text-slate-600'
                    }`}
                  >
                    {b.label}
                  </button>
    ...
    return (
      <div className="space-y-8 animate-in fade-in duration-500">
        ...
        <div className="bg-white rounded-2xl border border-slate-200 shadow-sm overflow-hidden">
          ...
          <button onClick={() => setShowAddMenu(!showAddMenu)} className="w-full py-4 border-t flex items-center justify-center space-x-2 text-slate-500"><Plus className="w-5 h-5" /><span>Add Menu Item</span></button>
        </div>
      </div>
    );
  }
  ...
  return (
    <div className="space-y-8 pb-20 animate-in fade-in duration-500">
      ...
      <div className="grid grid-cols-1 gap-4">
        ...
        {(items as PageElement[]).map((item, index) => (
          <div key={item.id} className="group relative bg-white border border-slate-200 rounded-2xl p-5 shadow-sm hover:border-indigo-300 transition-all flex items-center space-x-5">
            ...
            <div className="flex-1 min-w-0">
              ...
              <p className="text-xs text-slate-400 uppercase font-black tracking-tighter">{item.type === 'WIDGET' ? 'Qumu Video' : item.type}</p>
            </div>
            <div className="flex items-center space-x-2 opacity-0 group-hover:opacity-100 transition-opacity">
              {item.type === 'WIDGET' && onConfigureWidget && (
                <button 
                  onClick={() => onConfigureWidget('PLAYER', item.qumuConfig || {}, item)}
                  className="p-2.5 text-indigo-500 hover:text-indigo-700 hover:bg-indigo-50 rounded-xl transition-all"
                  title="Configure Qumu Video"
                >
        ...
        <button onClick={() => onAddElement(null)} className="w-full py-4 border-2 border-dashed border-slate-200 rounded-2xl flex items-center justify-center space-x-2 text-slate-400 hover:border-indigo-400 hover:text-indigo-600 transition-all font-bold"><Plus className="w-5 h-5" /><span>Add Element</span></button>
      </div>
    </div>
  );
};
