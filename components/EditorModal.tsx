...
import { X, Save, Type, FileText, PlaySquare, Code, Eye, ChevronLeft } from 'lucide-react';
...
const EditorModal: React.FC<EditorModalProps> = ({ section, element, onSave, onClose }) => {
  // If element is null, we start without a type to force choice
  ...
  const [formData, setFormData] = useState<PageElement>(element || {
    ...
    type: 'HTML', // Default, will be overwritten by selectedType
    title: '',
    content: '',
    widgetType: 'PLAYER'
  });
  ...
  const renderTypeSelection = () => (
    <div className="p-12 flex flex-col items-center justify-center flex-1 space-y-8">
      <div className="text-center space-y-2">
        ...
        <p className="text-slate-500">Choose the type of content you want to add to the {section}.</p>
      </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-6 w-full max-w-2xl">
        ...
        >
          ...
          <div className="space-y-1">
            <h4 className="font-bold text-slate-800">Video Widget</h4>
            <p className="text-xs text-slate-500">Embed Qumu video content</p>
          </div>
        </button>
      </div>
    </div>
          ...
          <div className="flex items-center space-x-3">
            ...
            <div>
              <h3 className="text-xl font-bold text-slate-800">
                {selectedType ? `${element ? 'Edit' : 'New'} ${selectedType}` : 'Add New Element'}
              </h3>
          ...
          <>
            <form onSubmit={handleSubmit} className="p-8 flex-1 overflow-y-auto space-y-6">
              <div className="space-y-2">
                ...
                <input 
                  type="text"
                  required
                  placeholder="e.g. Hero Section Text"
              ...
              {selectedType === 'WIDGET' ? (
                <div className="space-y-4 p-6 bg-slate-50 rounded-2xl border-2 border-slate-100">
                  <label className="text-sm font-bold text-slate-700 uppercase tracking-wider">Widget Display Configuration</label>
                  <div className="grid grid-cols-3 gap-3">
                      {(['PLAYER', 'GRID', 'CAROUSEL'] as const).map(wt => (
                        <button
                          key={wt}
                          type="button"
                          onClick={() => setFormData(p => ({ ...p, widgetType: wt }))}
                          className={`py-4 rounded-xl border-2 text-sm font-bold transition-all ${
                            formData.widgetType === wt 
                  ...
                  <p className="text-xs text-slate-400 italic">This will render the native Qumu video {formData.widgetType?.toLowerCase()} using your portal credentials.</p>
                </div>
              ) : (
                      ...
                      <div className="flex bg-slate-100 p-1 rounded-xl">
                        <button
                          ...
                          className={`flex items-center space-x-1.5 px-3 py-1 rounded-lg text-xs font-bold transition-all ${
                            editMode === 'visual' ? 'bg-white text-indigo-600 shadow-sm' : 'text-slate-500 hover:text-slate-700'
                          }`}
                        >
                        ...
                        <button
                          ...
                          className={`flex items-center space-x-1.5 px-3 py-1 rounded-lg text-xs font-bold transition-all ${
                            editMode === 'code' ? 'bg-white text-indigo-600 shadow-sm' : 'text-slate-500 hover:text-slate-700'
                          }`}
                        >
                  ...
                  <div className="relative">
                    {selectedType === 'HTML' && editMode === 'visual' ? (
                      ...
                      <textarea 
                        required
                        rows={12}
                        placeholder={selectedType === 'HTML' ? '<div>Your custom HTML here...</div>' : '# Markdown Title\nYour text here...'}
                        className={`w-full px-4 py-4 bg-slate-50 border-2 border-slate-100 rounded-2xl focus:border-indigo-500 focus:ring-0 transition-all font-mono text-sm leading-relaxed ${selectedType === 'HTML' ? 'text-indigo-900' : 'text-slate-700'}`}
                        value={formData.content}
            ...
            <div className="px-8 py-6 bg-slate-50 border-t shrink-0 flex justify-end space-x-3">
              ...
              <button 
                ...
                className="px-8 py-2.5 bg-indigo-600 text-white font-bold rounded-xl hover:bg-indigo-700 shadow-lg shadow-indigo-600/20 transition-all flex items-center space-x-2"
              >
